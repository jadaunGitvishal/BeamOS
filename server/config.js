...
function parseBillingRateTable(raw) {
  ...
  try {
    ...
    if (
      ...
      t.length &&
      t.every(
        (r) => typeof r.minScreens === "number" && typeof r.rate === "number",
      )
    )
      return t;
  } catch (_) {
...
module.exports = {
  ...
  disableHomepage: ["true", "1"].includes(
    String(process.env.DISABLE_HOMEPAGE || "").toLowerCase(),
  ),
  ...
  autoCreateOrgOnSignup: !["false", "0"].includes(
  ...
  lagTelemetryRetentionDays:
    parseFloat(process.env.LAG_TELEMETRY_RETENTION_DAYS) || 3,
  ...
  reconnectBaseBackoffMs:
    parseInt(process.env.RECONNECT_BASE_BACKOFF_MS) || 1000,
  reconnectMaxBackoffMs:
    parseInt(process.env.RECONNECT_MAX_BACKOFF_MS) || 60000,
  ...
  reconnectIdleResetMs:
    parseInt(process.env.RECONNECT_IDLE_RESET_MS) || 60 * 60 * 1000,
  ...
  connectRateCooldownMs:
    parseInt(process.env.CONNECT_RATE_COOLDOWN_MS) || 60000, // refuse window after a trip
  ...
  connectRateQuarantineTrips:
    parseInt(process.env.CONNECT_RATE_QUARANTINE_TRIPS) || 5,
  connectRateQuarantineMs:
    parseInt(process.env.CONNECT_RATE_QUARANTINE_MS) || 30 * 60 * 1000,
  ...
  reconnectBandElevatedMult:
    parseFloat(process.env.RECONNECT_BAND_ELEVATED_MULT) || 2,
  reconnectBandCriticalMult:
    parseFloat(process.env.RECONNECT_BAND_CRITICAL_MULT) || 4,
  ...
  statusLogRetentionDays:
    parseFloat(process.env.STATUS_LOG_RETENTION_DAYS) || 3,
  ...
  statusLogMaxRowsPerDevice:
    parseInt(process.env.STATUS_LOG_MAX_ROWS_PER_DEVICE) || 500,
  ...
  maintenanceBandGateEnabled:
    process.env.MAINTENANCE_BAND_GATE_ENABLED !== "false",
  ...
  otaDownloadMaxConcurrent:
    parseInt(process.env.OTA_DOWNLOAD_MAX_CONCURRENT) || 10,
  otaDownloadMaxPerWindow:
    parseInt(process.env.OTA_DOWNLOAD_MAX_PER_WINDOW) || 120,
  ...
  billing: {
    ...
    rateTable: parseBillingRateTable(process.env.BILLING_RATE_TABLE) || [
      { minScreens: 1, rate: 1.5 },
      { minScreens: 500, rate: 1.25 },
      { minScreens: 1000, rate: 1.0 },
    ],
    ...
    usageRetentionDays:
      parseInt(process.env.BILLING_USAGE_RETENTION_DAYS) || 400,
  ...
  walCheckpointIntervalMs:
    parseInt(process.env.WAL_CHECKPOINT_INTERVAL_MS) || 15000,
  ...
  walCheckpointHighWaterMB:
    parseInt(process.env.WAL_CHECKPOINT_HIGH_WATER_MB) || 16,
  ...
  walCheckpointStarvationRuns:
    parseInt(process.env.WAL_CHECKPOINT_STARVATION_RUNS) || 3,
  ...
  walCheckpointRespawnMax:
    parseInt(process.env.WAL_CHECKPOINT_RESPAWN_MAX) || 5,
  walCheckpointRespawnWindowMs:
    parseInt(process.env.WAL_CHECKPOINT_RESPAWN_WINDOW_MS) || 60000,
  walCheckpointRespawnBackoffMs:
    parseInt(process.env.WAL_CHECKPOINT_RESPAWN_BACKOFF_MS) || 1000,
  walCheckpointFallbackPages:
    parseInt(process.env.WAL_CHECKPOINT_FALLBACK_PAGES) || 1000,
  ...
  contentAckMaxPerWindow:
    parseInt(process.env.CONTENT_ACK_MAX_PER_WINDOW) || 20,
  contentAckRateWindowMs:
    parseInt(process.env.CONTENT_ACK_RATE_WINDOW_MS) || 10000,
  ...
  reclaimRejectLogWindowMs:
    parseInt(process.env.RECLAIM_REJECT_LOG_WINDOW_MS) || 60000,
};
