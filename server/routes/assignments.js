...
async function emitMuteChanged(req, item, muted) {
  try {
    ...
    const pl = await db.prepare('SELECT published_snapshot FROM playlists WHERE id = ?').get(item.playlist_id);
    ...
    const devices = await db.prepare('SELECT id FROM devices WHERE playlist_id = ?').all(item.playlist_id);
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  if (updates.length > 0) {
    ...
    if (mutedChanged) await emitMuteChanged(req, item, muted ? 1 : 0);
  }
...
router.post('/device/:deviceId/reorder', asyncHandler(async (req, res) => {
  ...
  const transaction = db.transaction(async (tx) => {
    ...
    for (let index = 0; index < order.length; index++) {
      await updateStmt.run(index, order[index], device.playlist_id);
    }
  });
  await transaction();
...
router.post('/device/:deviceId/copy-to/:targetDeviceId', asyncHandler(async (req, res) => {
  ...
  const maxRow = await db.prepare('SELECT MAX(sort_order) as m FROM playlist_items WHERE playlist_id = ?')
    .get(targetPlaylistId);
  const maxOrder = maxRow.m || 0;

  const transaction = db.transaction(async (tx) => {
    const stmt = tx.prepare('INSERT INTO playlist_items (playlist_id, content_id, widget_id, zone_id, sort_order, duration_sec) VALUES (?, ?, ?, ?, ?, ?)');
    for (let i = 0; i < sourceItems.length; i++) {
      const a = sourceItems[i];
      await stmt.run(targetPlaylistId, a.content_id, a.widget_id, a.zone_id || null, maxOrder + i + 1, a.duration_sec);
    }
  });
  await transaction();
