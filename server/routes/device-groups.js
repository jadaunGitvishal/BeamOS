...
const requireGroupRead = asyncHandler(async (req, res, next) => {
...
const requireGroupWrite = asyncHandler(async (req, res, next) => {
  const access = await loadGroupAccessCtx(req, res);
  if (!access) return;
...
// Update group
router.put('/:id', requireGroupWrite, asyncHandler(async (req, res) => {
  const { name, color } = req.body;
  if (color && !VALID_COLOR.test(color)) return res.status(400).json({ error: 'invalid color format, use #RRGGBB' });
...
router.delete('/:id', requireGroupWrite, asyncHandler(async (req, res) => {
  ...
  const convert = db.transaction(async (tx) => {
    // Find group schedules that need conversion
    const groupSchedules = await tx.prepare('SELECT * FROM schedules WHERE group_id = ?').all(groupId);

    // Find current group members
    const members = await tx.prepare('SELECT device_id FROM device_group_members WHERE group_id = ?').all(groupId);
    ...
    await tx.prepare('DELETE FROM schedules WHERE group_id = ?').run(groupId);

    // Delete the group (cascades to device_group_members)
    await tx.prepare('DELETE FROM device_groups WHERE id = ?').run(groupId);
...
router.get('/:id/devices', requireGroupRead, asyncHandler(async (req, res) => {
...
router.post('/:id/devices', requireGroupWrite, asyncHandler(async (req, res) => {
  ...
  try {
    await db.prepare('INSERT IGNORE INTO device_group_members (device_id, group_id) VALUES (?, ?)').run(device_id, req.params.id);
...
// db-handle-generic (works with the outer db OR a transaction-scoped tx handle, since
// both expose the same .prepare().get/.run() shape) - callers running inside a
// db.transaction() MUST pass their tx handle so these writes join that transaction.
async function ensureDevicePlaylistTx(dbh, deviceId, userId) {
  const device = await dbh.prepare('SELECT playlist_id, workspace_id, name FROM devices WHERE id = ?').get(deviceId);
  if (device?.playlist_id) return device.playlist_id;
  const playlistId = uuidv4();
  await dbh.prepare('INSERT INTO playlists (id, user_id, workspace_id, name, is_auto_generated) VALUES (?, ?, ?, ?, 1)')
    .run(playlistId, userId, device?.workspace_id || null, `${device?.name || 'Display'} playlist`);
  await dbh.prepare('UPDATE devices SET playlist_id = ? WHERE id = ?').run(playlistId, deviceId);
  return playlistId;
}
...
router.post('/:id/assign-content', requireGroupWrite, asyncHandler(async (req, res) => {
  ...
  // db.transaction()'s callback gets a transaction-scoped handle (tx) - statements
  // must be prepared from it, not the outer db, to actually participate in the transaction.
  // ensureDevicePlaylist/markDraft still use the outer db (not tx) for their own reads/
  // writes outside this transaction's scope where called elsewhere; here they're called
  // WITH tx so their writes join this transaction.
  const transaction = db.transaction(async (tx) => {
    for (const m of members) {
      const playlistId = await ensureDevicePlaylistTx(tx, m.device_id, req.user.id);
      const max = await tx.prepare('SELECT COALESCE(MAX(sort_order),0)+1 as next FROM playlist_items WHERE playlist_id = ?').get(playlistId);
      await tx.prepare('INSERT INTO playlist_items (playlist_id, content_id, sort_order, duration_sec) VALUES (?, ?, ?, ?)')
        .run(playlistId, content_id, max.next, duration_sec || 10);
      await tx.prepare("UPDATE playlists SET status = 'draft', updated_at = UNIX_TIMESTAMP() WHERE id = ?").run(playlistId);
    }
  });
  await transaction();
...
router.post('/:id/assign-playlist', requireGroupWrite, asyncHandler(async (req, res) => {
  ...
  // must be prepared from it, not the outer db, to actually participate in the transaction.
  const transaction = db.transaction(async (tx) => {
    const stmt = tx.prepare('UPDATE devices SET playlist_id = ? WHERE id = ?');
    await tx.prepare('UPDATE device_groups SET playlist_id = ? WHERE id = ?').run(playlist_id, req.params.id);
...
router.post('/:id/command', requireScope('full'), requireGroupWrite, asyncHandler(async (req, res) => {
