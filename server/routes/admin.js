...
router.post("/orgs", requirePlatformAdmin, (req, res) => {
  ...
  if (name.length > 120)
    return res
      .status(400)
      .json({ error: "Organization name must be 120 characters or fewer" });
...
router.put("/users/:id/workspace", requirePlatformAdmin, (req, res) => {
  ...
  if (memberships.length === 1 && memberships[0].workspace_id === ws.id) {
    ...
    return res.json({
      ...
      role: cur ? cur.role : "workspace_viewer",
      unchanged: true,
    });
  }
  ...
  logActivity(
    ...
    `target: ${target.email}, workspace: ${ws.id}`,
    null,
    getClientIp(req),
    ws.id,
  );
...
router.post("/users/:id/workspaces", requirePlatformAdmin, (req, res) => {
  ...
  logActivity(
    req.user.id,
    "admin_add_user_workspace",
    `target: ${target.email}, workspace: ${ws.id}, role: ${role}`,
    null,
    getClientIp(req),
    ws.id,
  );
...
router.put(
  ...
  (req, res) => {
    ...
    logActivity(
      req.user.id,
      "admin_set_user_workspace_role",
      `target: ${target?.email}, workspace: ${req.params.workspaceId}, role: ${role}`,
      null,
      getClientIp(req),
...
router.delete(
  ...
  (req, res) => {
    ...
    logActivity(
      req.user.id,
      "admin_remove_user_workspace",
      `target: ${target?.email}, workspace: ${req.params.workspaceId}`,
      null,
      getClientIp(req),
      req.params.workspaceId,
    );
  ...
  if (existing) {
    ...
    for (const f of BRANDING_FIELDS) {
      if (req.body[f] !== undefined) {
        updates.push(`${f} = ?`);
        values.push(
          f === "hide_branding" ? (req.body[f] ? 1 : 0) : req.body[f] || null,
        );
      }
    }
    if (updates.length) {
    ...
    `,
      ...
      PLATFORM_DEFAULT_ID,
      req.user.id,
      req.body.brand_name || "BeamOS",
      req.body.logo_url || null,
      req.body.favicon_url || null,
      req.body.primary_color || "#3B82F6",
      req.body.secondary_color || "#1E293B",
      req.body.bg_color || "#111827",
      req.body.custom_css || null,
      req.body.hide_branding ? 1 : 0,
    );
  }
  logActivity(
    req.user.id,
    "admin_set_platform_branding",
...
router.get("/status-debug", requirePlatformAdmin, (req, res) => {
  res.json({
    enabled: appSettings.getBool(
      "status_debug_enabled",
      config.statusDebugEnabled,
    ),
  });
});
