...
router.get('/playlists', asyncHandler(async (req, res) => {
  res.json(await listDesignatedPlaylists(db, req.apiToken.id, req.jwtWorkspaceId));
}));
...
router.get('/playlists/:playlistId/layout', asyncHandler(async (req, res) => {
  res.json(await listLayoutGeometry(db, req.apiToken.id, req.jwtWorkspaceId, req.params.playlistId));
}));
...
router.param('playlistId', asyncHandler(async (req, res, next, playlistId) => {
...
router.post('/playlists/:playlistId/items', asyncHandler(async (req, res) => {
  ...
  if (await isZonedPlaylist(db, req.params.playlistId)) {
  ...
  if (req.apiToken.auto_publish) {
    ...
    await db.prepare("UPDATE playlists SET status = 'draft', updated_at = UNIX_TIMESTAMP() WHERE id = ?").run(req.params.playlistId);
  }
