...
// Ref 73: query logic extracted out of routes/dashboard-reports.js's
// GET /sla-overview and GET /sla-trend handlers so it has exactly one home,
// callable without going through Express - the same reasoning as
// lib/ticket-query.js's extraction for tickets. The routes now just call
// these and res.json() the result; behavior/shape is unchanged.

const config = require('../config');
const appSettings = require('./app-settings');
const { getWorkspaceDeviceSubquery } = require('./workspace-scope');
const { detectOutages } = require('./outage-detection');
const { deviceAvailabilityRows } = require('./sla');
...
async function buildSlaOverview(db, req) {
...
async function buildSlaTrend(db, req) {
...
module.exports = { buildSlaOverview, buildSlaTrend };
