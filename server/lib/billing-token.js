...
async function resolveOwner(db) {
  return db.prepare(
    "SELECT id, email FROM users WHERE role IN ('platform_admin','superadmin') ORDER BY created_at ASC, id ASC LIMIT 1"
  ).get();
}
...
async function resolveWorkspaceId(db, ownerId) {
  ...
  // No rowid in MySQL (unlike SQLite) - order by created_at to preserve the "pick the
  // oldest workspace" fallback intent instead of an arbitrary/meaningless UUID sort.
  const any = await db.prepare('SELECT id FROM workspaces ORDER BY created_at ASC LIMIT 1').get();
...
async function mintBillingToken(db, { name } = {}) {
  ...
  const workspaceId = await resolveWorkspaceId(db, owner.id);
...
async function revokeBillingToken(db, id) {
  const row = await db.prepare('SELECT id, scope, revoked_at FROM api_tokens WHERE id = ?').get(id);
  ...
  await db.prepare("UPDATE api_tokens SET revoked_at = UNIX_TIMESTAMP() WHERE id = ?").run(id);
  return { ok: true };
}

async function listBillingTokens(db) {
  return db.prepare(
