...
function startAlertService(io) {
  // checkOfflineDevices is async; setInterval doesn't await it, so .catch() prevents a
  // rejection from becoming an unhandled rejection (server.js's crash handler is fatal).
  setInterval(() => {
    checkOfflineDevices(io).catch((e) =>
      console.error("[alerts] tick failed:", e.message),
    );
  }, 60000);
  ...
  for (const device of offlineDevices) {
    ...
    if (device.owner_email) {
      ...
      await sendEmail({
        ...
        console.error("[ALERT] sendEmail rejected unexpectedly:", e.message),
      );
...
// BeamOS-branded HTML body for alert emails. Owns the visual template
...
function buildAlertHtml(recipientName, subject, body) {
  return `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px">
    <h2 style="color:#3b82f6">BeamOS Alert</h2>
    <p>Hi ${escapeHtml(recipientName || "there")},</p>
    <div style="background:#f1f5f9;padding:16px;border-radius:8px;margin:16px 0">
      <strong>${escapeHtml(subject)}</strong><br><br>
      ${escapeHtml(body).replace(/\n/g, "<br>")}
    </div>
    <p style="color:#94a3b8;font-size:12px">You're receiving this because you have email alerts enabled in BeamOS.</p>
  </div>`;
}
...
// Internally delegates to sendEmail() with the BeamOS HTML template.
