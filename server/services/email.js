...
async function getAccessToken() {
  ...
  if (!result || !result.accessToken)
    throw new Error("No accessToken returned from MSAL");
  const expiresAtMs = result.expiresOn
    ? result.expiresOn.getTime()
    : Date.now() + 3_300_000; // 55min fallback
      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 200 && res.statusCode < 300) resolve();
            ...
            reject(
              new Error(
                `Graph sendMail ${res.statusCode}: ${chunks.slice(0, 500)}`,
              ),
            );
        });
      },
...
// rawSubject: when true, the subject is sent verbatim (no "[BeamOS] "
...
async function sendEmail({ to, subject, text, html, fromName, rawSubject }) {
  ...
  if (config.graphDevRestrictTo) {
    ...
    if (!allowed.includes(String(to).toLowerCase())) {
      console.log(
        `[EMAIL] dev restrict - would send to ${to}: ${subject} (suppressed)`,
      );
