...
# Best-effort: report the running version. Tries HTTP :5001 then HTTPS :3443.
echo "==> Waiting for the service to answer..."
OUT=""
for i in $(seq 1 30); do
  for URL in "${STATUS_URL:-}" http://localhost:5001/api/status https://localhost:3443/api/status; do
    [ -z "$URL" ] && continue
