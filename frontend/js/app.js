...
function mapAcceptError(err) {
  ...
  if (/Invite has expired|Workspace no longer exists/i.test(msg))
    return t("accept.error.expired");
  if (/different email address/i.test(msg))
    return t("accept.error.wrong_account");
...
async function consumeAcceptInvite(inviteId) {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "switchWorkspace after accept failed (non-fatal):",
        e.message,
      );
    }
    ...
    const toastKey = result.already_member
      ? "accept.already_member"
      : "accept.success";
    localStorage.setItem(
      PENDING_INVITE_TOAST_KEY,
      JSON.stringify({
...
async function refreshCurrentUser() {
  ...
  try {
    ...
    if (
      hasNoAccessibleWorkspace(fresh) &&
      hash !== "#/no-workspace" &&
      hash !== "#/login" &&
...
function route() {
  ...
  if (isAuthenticated() && hash === "#/login") {
    window.location.hash = localStorage.getItem("rd_onboarded")
      ? "#/"
      : "#/onboarding";
    return;
  }
  ...
  navLinks.forEach((link) => {
    ...
    if (hash === "#/" && link.dataset.view === "dashboard")
      link.classList.add("active");
    else if (hash.startsWith("#/content") && link.dataset.view === "content")
      link.classList.add("active");
    else if (hash.startsWith("#/settings") && link.dataset.view === "settings")
      link.classList.add("active");
    else if (hash.startsWith("#/billing") && link.dataset.view === "billing")
      link.classList.add("active");
    else if (
      (hash.startsWith("#/layout") || hash === "#/layouts") &&
      link.dataset.view === "layouts"
    )
      link.classList.add("active");
    else if (
      (hash === "#/playlists" || hash.startsWith("#/playlists/")) &&
      link.dataset.view === "playlists"
    )
      link.classList.add("active");
    else if (hash === "#/schedule" && link.dataset.view === "schedule")
      link.classList.add("active");
    else if (hash === "#/widgets" && link.dataset.view === "widgets")
      link.classList.add("active");
    else if (
      (hash.startsWith("#/wall") || hash === "#/walls") &&
      link.dataset.view === "walls"
    )
      link.classList.add("active");
    else if (hash === "#/reports" && link.dataset.view === "reports")
      link.classList.add("active");
    else if (hash === "#/activity" && link.dataset.view === "activity")
      link.classList.add("active");
    else if (hash === "#/designer" && link.dataset.view === "designer")
      link.classList.add("active");
    else if (
      (hash === "#/kiosk" || hash.startsWith("#/kiosk/")) &&
      link.dataset.view === "kiosk"
    )
      link.classList.add("active");
    else if (hash === "#/help" && link.dataset.view === "help")
      link.classList.add("active");
    else if (hash.startsWith("#/device/") && link.dataset.view === "dashboard")
      link.classList.add("active");
  });
  ...
  if (hash === "#/" || hash === "#" || hash === "") {
    ...
    // Subscription is permanently disabled for BeamOS - always bounce to dashboard,
    // regardless of what the server's hide_billing flag says.
    history.replaceState(null, "", window.location.pathname + "#/");
    currentView = dashboard;
    dashboard.render(app);
  } else {
...
function updateSidebarUser() {
  ...
  const billingNav = document.getElementById("billingNavItem");
  if (billingNav) billingNav.style.display = "none"; // Subscription permanently disabled for BeamOS
  ...
  userEl.innerHTML = `
    ${
      user.avatar_url
        ? `<img src="${user.avatar_url}" style="width:28px;height:28px;border-radius:50%">`
  ...
  try {
    ...
    if (hash !== knownHash) {
      ...
      if (toast) {
        ...
        notice.innerHTML =
          '<span>Dashboard updated. <a href="javascript:location.reload()" style="color:var(--accent);text-decoration:underline;font-weight:600">Reload now</a></span>';
