...
async function loadLoginBranding() {
  try {
    const res = await fetch(
      "/api/branding?domain=" + encodeURIComponent(location.hostname),
    );
    if (!res.ok) return {};
...
function applyLoginBrandingDoc(b) {
  ...
  if (b.favicon_url) {
    document
      ...
      .forEach((l) => l.setAttribute("href", b.favicon_url));
  }
  if (b.custom_css) {
...
export async function render(container) {
  ...
  //         </svg>`;
  const logoHtml = `<img src="/assets/icon-192-v2.png" alt="${brandEsc(brandName)}" style="max-height:48px;max-width:200px;margin:0 auto 12px;display:block">`;
...
function setupHandlers(config, isSetup) {
  ...
  // Support token login
  document
    .getElementById("supportLoginBtn")
    ?.addEventListener("click", async () => {
      const token = document.getElementById("supportToken")?.value.trim();
      if (!token) {
        showError(t("auth.error_paste_support_token"));
        return;
      }
      try {
        ...
        showError(t("auth.error_support_failed"));
      }
    });
  ...
  if (isSetup) {
    document
      ...
      ?.addEventListener("click", () => doRegister(true));
  } else {
    ...
    document
      ...
      ?.addEventListener("click", () => doRegister(false));
  }
  ...
  async function doLogin() {
    ...
    if (!email || !password) {
      showError(t("auth.error_email_password_required"));
      return;
    }
  ...
  async function doRegister(isFirstUser) {
    const email = document
      .getElementById(isFirstUser ? "loginEmail" : "regEmail")
      .value.trim();
    const password = document.getElementById(
      isFirstUser ? "loginPassword" : "regPassword",
    ).value;
    const name =
      document
        .getElementById(isFirstUser ? "loginName" : "regName")
        ?.value.trim() || "";
    if (!email || !password) {
      showError(t("auth.error_email_password_required"));
      return;
    ...
    if (password.length < 6) {
      showError(t("auth.error_password_min_6"));
      return;
    }
