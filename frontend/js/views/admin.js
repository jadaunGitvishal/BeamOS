...
export async function render(container) {
  ...
  document.getElementById("adminAddUserBtn")?.addEventListener("click", () => {
    openAddUserModal(null, {
      onSuccess: (result) => {
        showToast(
          t("members.success.user_created", { email: result.email }),
          "success",
        );
        loadUsers();
      },
  ...
  document
    .getElementById("adminCreateOrgBtn")
    ?.addEventListener("click", () => {
      openCreateOrgModal({
        onSuccess: (result) =>
          showToast(
            t("admin.create_org.success", { name: result.name }),
            "success",
          ),
      });
    });
...
async function loadOrgs() {
  ...
  el.querySelectorAll("[data-del-ws]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const id = btn.dataset.delWs,
        name = btn.dataset.wsName;
      openTypeToConfirmModal({
...
async function loadBranding() {
  ...
  let b = {};
  try {
    b = await api.adminGetBranding();
  } catch (e) {
    el.innerHTML = `<p style="color:var(--danger)">${esc(e.message || "Failed to load")}</p>`;
    return;
  }
  ...
  try {
    ...
    el.querySelectorAll("[data-role-user]").forEach((select) => {
      select.onchange = async () => {
        try {
          await API(`/auth/users/${select.dataset.roleUser}/role`, {
            method: "PUT",
    ...
    el.querySelectorAll("[data-plan-user]").forEach((select) => {
      select.onchange = async () => {
        try {
          await API("/subscription/assign", {
            method: "POST",
            body: JSON.stringify({
              user_id: select.dataset.planUser,
    ...
    el.querySelectorAll("[data-reset-pw-user]").forEach((btn) => {
      btn.onclick = async () => {
        ...
        if (pw === null) return;
        if (pw.length < 8) {
          showToast(t("admin.toast.password_min_8"), "error");
          return;
        }
        try {
      ...
      btn.onclick = async () => {
        if (confirming) {
          try {
            ...
            showToast(t("admin.toast.user_removed"), "success");
            loadUsers();
          } catch (err) {
            showToast(err.message, "error");
          }
        ...
        setTimeout(() => {
          confirming = false;
          btn.textContent = t("admin.remove");
...
async function loadStatusDebug() {
  ...
  let enabled = false;
  try {
    enabled = (await api.adminGetStatusDebug()).enabled;
  } catch (e) {
    el.innerHTML = `<p style="color:var(--danger)">${esc(e.message || "Failed to load")}</p>`;
    return;
  }
  ...
  document.getElementById("statusDebugChk").onchange = async (e) => {
    ...
    try {
      ...
      showToast(
        "Status debug " + (chk.checked ? "enabled" : "disabled"),
        "success",
      );
    } catch (err) {
      showToast(err.message, "error");
