...
export async function render(container) {
  ...
  try {
    appVersion =
      (await fetch("/api/version").then((r) => r.json())).version || "";
  } catch {
  ...
  container.innerHTML = `
    ...
    <div class="settings-section" id="whiteLabelSection">
      ...
      <div id="whiteLabelForm">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="form-group"><label>${t("settings.brand_name")}</label><input type="text" id="wlBrandName" class="input" placeholder=""></div>
    ...
    <div class="settings-section">
      ...
      <select id="langSelect" class="input" style="width:200px;background:var(--bg-input)">
        ${getAvailableLanguages()
          .map(
            (l) =>
              `<option value="${l.code}" ${l.code === getLanguage() ? "selected" : ""}>${l.name}</option>`,
          )
          .join("")}
      </select>
    </div>
    ...
    <div class="settings-section">
      ...
      <div style="color:var(--text-secondary);font-size:13px">
        <p><strong>BeamOS</strong>${appVersion ? ` v${esc(appVersion)}` : ""}</p>
  ...
  if (isAdmin) {
    ...
    document
      .getElementById("generateSupportBtn")
      ?.addEventListener("click", async () => {
        const org =
          document.getElementById("supportOrg").value.trim() || "Customer";
        const hours =
          parseInt(document.getElementById("supportHours").value) || 4;
        try {
          ...
          if (res.ok) {
            ...
            document.getElementById("supportTokenResult").style.display =
              "block";
            showToast(
              t("settings.toast.support_token_generated", { hours }),
              "success",
            );
    ...
    ?.addEventListener("change", async (e) => {
      ...
      try {
        ...
        if (isZip) {
          ...
          data = { format: "BeamOS-export-v1", _isZip: true };
          ...
          if (!data.format || !data.format.startsWith("BeamOS-export")) {
          ...
          const summary = [
            data.devices?.length
              ? t("settings.import.summary_devices", { n: data.devices.length })
              : null,
            data.content?.length
              ? t("settings.import.summary_content", { n: data.content.length })
              : null,
            data.widgets?.length
              ? t("settings.import.summary_widgets", { n: data.widgets.length })
              : null,
            data.layouts?.length
              ? t("settings.import.summary_layouts", { n: data.layouts.length })
              : null,
            data.schedules?.length
              ? t("settings.import.summary_schedules", {
                  n: data.schedules.length,
                })
              : null,
            data.video_walls?.length
              ? t("settings.import.summary_walls", {
                  n: data.video_walls.length,
                })
              : null,
            data.kiosk_pages?.length
              ? t("settings.import.summary_kiosk", {
                  n: data.kiosk_pages.length,
                })
              : null,
          ]
            .filter(Boolean)
        ...
        document.getElementById("confirmImportBtn").onclick = async () => {
          statusEl.innerHTML = isZip
            ? t("settings.import.uploading_zip")
          ...
          try {
            ...
            if (res.ok) {
              const imported = Object.entries(result.stats)
                .filter(([k, v]) => v > 0 && k !== "files_restored")
                .map(([k, v]) => `${v} ${k}`)
                .join(", ");
              ...
              if (result.device_pairings?.length) {
                html +=
                  `<br><br><strong>${t("settings.import.pairing_codes_title")}</strong><br><table style="margin-top:8px;font-size:12px;border-collapse:collapse">` +
                  result.device_pairings
                    .map(
                      (d) =>
                        `<tr><td style="padding:4px 12px 4px 0">${d.name}</td><td style="font-family:monospace;font-weight:700;font-size:14px;letter-spacing:2px">${d.pairing_code}</td></tr>`,
                    )
                    .join("") +
              ...
              statusEl.textContent =
                result.error || t("settings.import.failed");
            }
          } catch (err) {
            statusEl.style.color = "var(--danger)";
            statusEl.textContent = t("settings.import.failed_with_error", {
              error: err.message,
            });
          }
          e.target.value = "";
        ...
        statusEl.style.color = "var(--danger)";
        statusEl.textContent = t("settings.import.read_failed", {
          error: err.message,
        });
      }
    });
  ...
  async function loadTokens() {
    ...
    el.querySelectorAll(".edit-targets-btn").forEach((btn) =>
      btn.addEventListener("click", async () => {
        const id = btn.dataset.id;
        const current = new Set(
          (btn.dataset.targets || "").split(",").filter(Boolean),
        );
                ...
                ? pls
                    .map((p) =>
                      p.zoned
                        ...
                        : `<label style="display:flex;gap:8px;align-items:center;font-size:13px"><input type="checkbox" class="edit-pl" value="${esc(String(p.id))}"${current.has(String(p.id)) ? " checked" : ""}> ${esc(p.name)}</label>`,
                    )
                    .join("")
        ...
        document.getElementById("saveTargetsBtn").onclick = async () => {
          const ids = [...panel.querySelectorAll(".edit-pl:checked")].map(
            (c) => c.value,
          );
          if (!ids.length)
            return showToast(t("apitoken.agency_needs_playlists"), "error");
          try {
        ...
        document.getElementById("cancelTargetsBtn").onclick = () => {
          panel.style.display = "none";
        };
      }),
    );
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      if (scope === "agency") {
        const ids = [
          ...document.querySelectorAll(
            "#agencyPlaylistList .agency-pl:checked",
          ),
        ].map((c) => c.value);
        if (!ids.length)
          return showToast(t("apitoken.agency_needs_playlists"), "error");
        ...
        payload.auto_publish =
          !!document.getElementById("tokAutoPublish")?.checked;
      }
      ...
      try {
        ...
        const inviteText = t("apitoken.invite_text", {
          url: portalUrl,
          key: r.token,
        });
        box.innerHTML = `
  ...
  document
    .getElementById("saveAcctBtn")
    ?.addEventListener("click", async () => {
      const name = document.getElementById("acctName").value.trim();
      if (!name) return showToast(t("settings.toast.name_required"), "error");
      const email_alerts =
        !!document.getElementById("acctEmailAlerts")?.checked;
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      if (!current)
        return showToast(
          t("settings.toast.current_password_required"),
          "error",
        );
      if (next.length < 8)
        return showToast(t("settings.toast.new_password_min_8"), "error");
      if (next !== confirm)
        return showToast(t("settings.toast.passwords_dont_match"), "error");
...
async function loadWhiteLabel() {
  ...
  try {
    ...
    if (wl.brand_name)
      document.getElementById("wlBrandName").value = wl.brand_name;
    if (wl.logo_url) document.getElementById("wlLogoUrl").value = wl.logo_url;
    if (wl.primary_color)
      document.getElementById("wlPrimaryColor").value = wl.primary_color;
    if (wl.bg_color) document.getElementById("wlBgColor").value = wl.bg_color;
    if (wl.custom_domain)
      document.getElementById("wlDomain").value = wl.custom_domain;
    if (wl.favicon_url)
      document.getElementById("wlFavicon").value = wl.favicon_url;
    if (wl.custom_css)
      document.getElementById("wlCustomCss").value = wl.custom_css;
    if (wl.hide_branding)
      document.getElementById("wlHideBranding").checked = true;
  } catch {}

  document
    .getElementById("saveWhiteLabelBtn")
    ?.addEventListener("click", async () => {
      try {
        await fetch("/api/white-label", {
          ...
          body: JSON.stringify({
            ...
            hide_branding: document.getElementById("wlHideBranding").checked
              ? 1
              : 0,
          }),
        });
  ...
  document
    .getElementById("previewWhiteLabelBtn")
  ...
  try {
    ...
    el.querySelectorAll(".reset-user-pw-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (pw === null) return;
        if (pw.length < 8) {
          showToast(t("settings.toast.new_password_min_8"), "error");
          return;
        }
        try {
