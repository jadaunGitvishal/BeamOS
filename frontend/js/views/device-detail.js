...
function renderDeviceClock(device) {
  ...
  const skewSec = Math.abs(
    Math.round(device.reported_utc / 1000) - device.reported_at,
  );
  let local = "";
  try {
    local = new Date(device.reported_utc).toLocaleString(undefined, {
      timeZone: device.reported_timezone || undefined,
      hour: "2-digit",
      minute: "2-digit",
      month: "short",
      day: "numeric",
...
export function render(container, deviceId) {
  ...
  screenshotHandler = (data) => {
    ...
    const imgSrc =
      ...
      (() => {
        ...
        return (
          data.url + (data.url.includes("?") ? "&" : "?") + "token=" + token
        );
      })();
  ...
  playbackHandler = (data) => {
    ...
    if (el && data.current_content_id) {
      el.textContent = t("device.now_playing_id", {
        id: data.current_content_id,
      });
    }
  };
...
async function loadDevice(deviceId, activeTab = null) {
  ...
  try {
            ...
            <div class="info-card-value small">${
              device.screen_width && device.screen_height
                ? device.screen_width +
                  ...
                  (device.render_width &&
                  device.render_height &&
                  (device.render_width !== device.screen_width ||
                    device.render_height !== device.screen_height)
    ...
    window._enableSystemView = () => {
      ...
      if (btn) {
        btn.textContent = t("device.remote.waiting_for_approval");
        btn.disabled = true;
      }
      ...
      setTimeout(() => {
        ...
        if (btn) {
          btn.textContent = t("device.remote.system_view_enabled");
  ...
  return assignments
    ...
    <div class="playlist-item" data-assignment-id="${a.id}" draggable="true" data-sort="${i}">
      ...
      <div class="playlist-item-info">
        ...
        ${
          a.orphan
            ? `<div class="pl-orphan-warning" data-orphan-assignment="${a.id}" title="${t("device.pl_item.orphan_zone_tip")}" style="margin-top:4px;font-size:11px;color:var(--danger);cursor:pointer;display:inline-flex;align-items:center;gap:4px">
...
async function setupActions(device) {
  // #104 Preview button
  document
    .getElementById("devicePreviewBtn")
    ?.addEventListener("click", () => showDevicePreview(device));
    ...
    ?.addEventListener("click", async () => {
      try {
        await api.updateDevice(device.id, {
          ...
          default_content_id:
            document.getElementById("deviceDefaultContent").value || null,
        });
      ...
      .then((playlists) => {
        playlists.forEach((p) => {
          ...
          opt.textContent = p.is_auto_generated
            ? t("device.playlist_picker.with_auto", {
                name: p.name,
                n: p.item_count,
              })
            : t("device.playlist_picker.with_count", {
                name: p.name,
                n: p.item_count,
              });
  ...
  document
    .getElementById("copyPlaylistBtn")
    ?.addEventListener("click", async () => {
      try {
        const devices = await api.getDevices();
        const others = devices.filter((d) => d.id !== device.id);
        if (!others.length) {
          showToast(t("device.copy.no_other_devices"), "info");
          return;
        }

        const targetId = prompt(
          t("device.copy.prompt", {
            list: others.map((d, i) => `${i + 1}. ${d.name}`).join("\n"),
          }),
        );
        if (!targetId) return;
        const target = others[parseInt(targetId) - 1];
        if (!target) {
          showToast(t("device.copy.invalid_selection"), "error");
          return;
        }

        const token = localStorage.getItem("token");
        const res = await fetch(
          `/api/assignments/device/${device.id}/copy-to/${target.id}`,
          {
            method: "POST",
            headers: {
        ...
        if (res.ok)
          showToast(
            t("device.copy.toast", { n: data.copied, device: target.name }),
            "success",
          );
  ...
  blockBtn?.addEventListener("click", async () => {
    ...
    try {
      if (device.blocked) {
        ...
        device.blocked = 1;
        showToast("Device blocked — refused on next reconnect", "success");
      }
  ...
  function sendWithFeedback(type, cmdLabel, successKey) {
    sendCommand(device.id, type, {}, (ack) => {
      if (ack?.delivered) showToast(t(successKey), "success");
        ...
        showToast(
          t("device.toast.command_queued", { cmd: cmdLabel }),
          "warning",
        );
      else if (ack?.reason === "no_ack")
        showToast(t("device.toast.command_no_ack", { cmd: cmdLabel }), "error");
      else
        showToast(
          t("device.toast.command_undeliverable", { cmd: cmdLabel }),
          "error",
        );
    });
  }
  ...
  document.getElementById("sendPipBtn")?.addEventListener("click", async () => {
    ...
    if (!uri) {
      showToast("Enter an overlay URL", "error");
      return;
    }
    try {
      ...
      showToast(
        `Overlay sent (${res.sent} sent, ${res.offline} offline)`,
  ...
  document
    .getElementById("clearPipBtn")
    ?.addEventListener("click", async () => {
      try {
        await api.clearPip(device.id);
        showToast("Overlay cleared", "success");
      } catch (err) {
        showToast(err.message, "error");
      }
    });
    ...
    if (select) {
      layouts
        ...
        .forEach((l) => {
          const opt = document.createElement("option");
          opt.value = l.id;
          opt.textContent = t("device.layout.zones_count", {
            name: l.name,
            n: l.zones?.length || 0,
          });
      ...
      layouts
        ...
        .forEach((l) => {
          const opt = document.createElement("option");
          opt.value = l.id;
          opt.textContent = t("device.layout.template_zones_count", {
            name: l.name,
            n: l.zones?.length || 0,
          });
          if (device.layout_id === l.id) opt.selected = true;
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast(
          layoutId
            ? t("device.toast.layout_applied")
            : t("device.toast.switched_to_fullscreen"),
          "success",
        );
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (device.layout_id) {
          try {
            const res = await fetch(`/api/layouts/${device.layout_id}`, {
              headers,
            });
          ...
          <div class="modal-body">
            ...
            <div class="assign-content-grid" id="assignMedia">
              ...
              `,
                  ...
                  .join("") ||
                `<p style="color:var(--text-muted);padding:16px;text-align:center">${t("device.assign.no_media")}</p>`
              }
            </div>
            <!-- Widgets grid -->
            <div class="assign-content-grid" id="assignWidgets" style="display:none">
              ${
                ...
                <div class="assign-content-item" data-content-id="${w.id}" data-type="widget">
                  ...
                  .join("") ||
                `<p style="color:var(--text-muted);padding:16px;text-align:center">${t("device.assign.no_widgets")} <a href="#/widgets" style="color:var(--accent)">${t("device.assign.create_one")}</a></p>`
              }
            </div>
            <!-- Kiosk grid -->
            <div class="assign-content-grid" id="assignKiosk" style="display:none">
              ...
              `,
                  ...
                  .join("") ||
                `<p style="color:var(--text-muted);padding:16px;text-align:center">${t("device.assign.no_kiosk")} <a href="#/kiosk" style="color:var(--accent)">${t("device.assign.create_one")}</a></p>`
              }
            </div>
          </div>
      ...
      `;
        ...
        modal.querySelectorAll(".assign-tab").forEach((tab) => {
          tab.onclick = () => {
            modal.querySelectorAll(".assign-tab").forEach((t) => {
              t.style.borderBottomColor = "transparent";
              t.style.color = "var(--text-secondary)";
            });
            tab.style.borderBottomColor = "var(--accent)";
            tab.style.color = "var(--accent)";
            document.getElementById("assignMedia").style.display =
              tab.dataset.tab === "media" ? "" : "none";
            document.getElementById("assignWidgets").style.display =
              tab.dataset.tab === "widgets" ? "" : "none";
            document.getElementById("assignKiosk").style.display =
              tab.dataset.tab === "kiosk" ? "" : "none";
          };
        });
        ...
        modal.querySelectorAll(".assign-content-item").forEach((item) => {
          item.addEventListener("click", () => {
            modal
              .querySelectorAll(".assign-content-item")
        ...
        modal.querySelector("#confirmAssign").onclick = async () => {
          ...
          const duration =
            parseInt(modal.querySelector("#assignDuration").value) || 10;
          const zoneId = modal.querySelector("#assignZone")?.value || null;
          try {
            if (selectedType === "content") {
              await api.addAssignment(device.id, {
              ...
              const wRes = await fetch("/api/widgets", {
                ...
                body: JSON.stringify({
                  widget_type: "webpage",
                  name: t("device.assign.kiosk_widget_name", {
                    name:
                      kioskPages.find((k) => k.id === selectedId)?.name ||
                      "Page",
                  }),
                  config: {
                    url: `${serverUrl}/api/kiosk/${selectedId}/render`,
                  },
                }),
              });
...
function attachRemoveHandlers(device) {
  ...
  if (device.layout_id) {
    ...
    document.querySelectorAll(".zone-select").forEach((s) => {
      s.style.display = "";
    });
    ...
    document.querySelectorAll(".pl-orphan-warning").forEach((w) => {
      w.addEventListener("click", () => {
        const sel = document.querySelector(
          '.zone-select[data-assignment-id="' +
            w.dataset.orphanAssignment +
            '"]',
        );
        if (sel) {
    ...
    if (device.active_layout_zones && device.active_layout_zones.length) {
      ...
      const token = localStorage.getItem("token");
      fetch(`/api/layouts/${device.layout_id}`, {
        ...
        .catch((e) =>
          console.warn(
            "Zone dropdowns: layout fetch fallback failed:",
            e.message,
          ),
        );
    }
  ...
  document.querySelectorAll(".mute-toggle").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        showToast(
          currentlyMuted ? t("device.toast.unmuted") : t("device.toast.muted"),
          "success",
        );
        loadDevice(device.id, "playlist");
      } catch (err) {
  ...
  container.querySelectorAll(".playlist-item[draggable]").forEach((item) => {
    ...
    item.addEventListener("drop", async (e) => {
      ...
      // Get new order
      const items = [
        ...container.querySelectorAll(".playlist-item[data-assignment-id]"),
      ];
...
function renderUptimeTimeline(uptimeData, statusLog = []) {
  ...
  for (let i = 0; i < statusLog.length; i++) {
    ...
    const startSlot = Math.max(
      0,
      Math.floor((event.timestamp - dayAgo) / slotDuration),
    );
    const endSlot = nextEvent
      ? Math.min(
          slots - 1,
          Math.floor((nextEvent.timestamp - dayAgo) / slotDuration),
        )
      : event.status === "online"
        ? slots - 1
        : startSlot;
  ...
  const uptimePct =
    knownSlots > 0 ? Math.round((onlineSlots / knownSlots) * 100) : 0;
  if (percentEl) {
    percentEl.textContent =
      knownSlots > 0
        ? t("device.timeline.uptime_pct_tracked", {
            pct: uptimePct,
            n: knownSlots * 15,
          })
...
function updateTelemetryDisplay(telemetry) {
  ...
  if (telemetry.battery_level != null)
    update("telBattery", telemetry.battery_level + "%");
  if (telemetry.storage_free_mb)
    update(
      "telStorage",
      t("device.info.size_free", {
        size: formatBytes(telemetry.storage_free_mb),
      }),
    );
  if (telemetry.wifi_ssid) update("telWifi", telemetry.wifi_ssid);
  if (telemetry.wifi_rssi) update("telRssi", telemetry.wifi_rssi + " dBm");
  if (telemetry.uptime_seconds)
    update("telUptime", formatUptime(telemetry.uptime_seconds));
  if (telemetry.ram_free_mb)
    update(
      "telRam",
      t("device.info.size_free", { size: formatBytes(telemetry.ram_free_mb) }),
    );
