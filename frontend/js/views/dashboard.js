...
function formatTimeAgo(timestamp) {
  ...
  if (seconds < 3600)
    return t("common.minutes_ago", { n: Math.floor(seconds / 60) });
  if (seconds < 86400)
    return t("common.hours_ago", { n: Math.floor(seconds / 3600) });
...
function renderProgressFor(deviceId) {
  ...
  document
    .querySelectorAll(`#progress-${CSS.escape(deviceId)}`)
    .forEach((el) => {
      if (!state) {
  ...
  return `
    <div class="device-card${checked ? " selected" : ""}" draggable="true" data-device-id="${device.id}" data-device-name="${esc(device.name)}" onclick="window.location.hash='/device/${device.id}'">
      ...
      <div class="device-card-body">
        <div class="device-card-name">${esc(device.name)}${
...
function renderWallCard(wall) {
  ...
  for (let r = 0; r < wall.grid_rows; r++) {
    for (let c = 0; c < wall.grid_cols; c++) {
      const dev = (wall.devices || []).find(
        (d) => d.grid_col === c && d.grid_row === r,
      );
      cells.push(
        `<div class="wall-card-cell${dev ? " filled" : ""}" title="${dev ? esc(dev.device_name) : "[" + c + "," + r + "]"}"></div>`,
      );
    }
  }
  const onlineCount = (wall.devices || []).filter(
    (d) => d.device_status === "online",
  ).length;
  return `
...
export function render(container) {
  ...
  function filterDevices() {
    ...
    document.querySelectorAll(".device-card").forEach((card) => {
      const name =
        card.querySelector(".device-card-name")?.textContent.toLowerCase() ||
        "";
      const deviceStatus =
        card.querySelector(".device-card-status span:last-child")
          ?.textContent || "";
  ...
  document.getElementById("clearSelectionBtn").addEventListener("click", () => {
    ...
    document
      .querySelectorAll(".device-card.selected")
      .forEach((c) => c.classList.remove("selected"));
    refreshSelectionBar();
  });

  document
    .getElementById("createWallBtn")
    .addEventListener("click", () => createWallFromSelection());
  ...
  screenshotHandler = (data) => {
    document
      .querySelectorAll(`#preview-${data.device_id}`)
      .forEach((preview) => {
        const imgSrc =
          data.image_data ||
          data.url + "&token=" + localStorage.getItem("token");
...
async function createWallFromSelection() {
  ...
  if (ids.length < 2) {
    showToast("Select at least 2 displays", "error");
    return;
  }
  const name = prompt(
    "Name this video wall:",
    `Wall ${new Date().toLocaleString()}`,
  );
  if (!name) return;
...
async function loadDashboard() {
  ...
  try {
    ...
    const provisioning = devices.filter(
    ...
    for (const w of walls || [])
      for (const d of w.devices || []) walledDeviceIds.add(d.device_id);
...
function attachGroupHandlers(groupsWithDevices, allDevices) {
  ...
  document.querySelectorAll(".device-card").forEach((card) => {
    ...
    card.addEventListener("drop", async (e) => {
      ...
      if (!grid) return;
      const ids = Array.from(grid.querySelectorAll(".device-card"))
  ...
  document.querySelectorAll(".group-section").forEach((section) => {
    ...
    section.addEventListener("drop", async (e) => {
      ...
      const deviceName =
        e.dataTransfer.getData("text/device-name") || "this device";
      if (!deviceId) return;
      ...
      if (targetGroup.memberIds.has(deviceId)) {
        showToast(
          t("dashboard.toast.already_in_group", {
            name: deviceName,
      ...
      if (others.length > 0) {
        if (
          !confirm(
            t("dashboard.confirm_add_to_group", {
              name: deviceName,
              groups: others.join(", "),
      ...
      try {
        ...
        showToast(
          t("dashboard.toast.moved_device", {
            name: deviceName,
            group: targetGroup.name,
          }),
          "success",
  ...
  document.querySelectorAll('[data-ungrouped="1"]').forEach((section) => {
    ...
    section.addEventListener("drop", async (e) => {
      ...
      try {
        await Promise.all(
          memberships.map((m) => api.removeDeviceFromGroup(m.id, deviceId)),
        );
        showToast(
          tn("dashboard.toast.removed_device", memberships.length, {
            name: deviceName,
          }),
          "success",
        );
  ...
  document.querySelectorAll(".group-playlist-select").forEach((select) => {
    select.addEventListener("change", async (e) => {
      ...
      if (
        !confirm(
          t("dashboard.confirm_assign_playlist", {
      ...
      try {
        ...
        showToast(
          tn("dashboard.toast.playlist_assigned", result.devices_updated),
          "success",
        );
      } catch (err) {
        showToast(err.message, "error");
  ...
  document.querySelectorAll(".group-cmd-select").forEach((select) => {
    select.addEventListener("change", async (e) => {
      ...
      if (DESTRUCTIVE_COMMANDS.includes(type)) {
        if (
          !confirm(
            t("dashboard.confirm_destructive_command", {
              cmd: cmdLabel.toUpperCase(),
              n: count,
              group: groupName,
            }),
          )
      ...
      try {
        ...
        const msg =
          result.offline > 0
            ? t("dashboard.toast.command_sent_with_offline", {
                cmd: cmdLabel,
                sent: result.sent,
                total: result.total,
                offline: result.offline,
              })
            : t("dashboard.toast.command_sent", {
                cmd: cmdLabel,
                sent: result.sent,
                total: result.total,
              });
  ...
  document.querySelectorAll("[data-group-manage]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      modal.innerHTML = `
        <div style="background:var(--bg-card);border-radius:12px;padding:24px;max-width:400px;width:90%;max-height:70vh;overflow-y:auto">
          ...
          <div style="display:flex;flex-direction:column;gap:6px">
            ${allDevices
              .filter((d) => d.status !== "provisioning")
              .map((d) => {
                const inOther = otherGroups
                  .filter((g) => g.memberIds.has(d.id))
                  .map((g) => g.name);
                return `
      ...
      modal.querySelector("#manageGroupClose").onclick = () => {
        modal.remove();
        loadDashboard();
      };
      ...
      modal.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
        cb.addEventListener("change", async () => {
          ...
          const cbName =
            cb.closest("label")?.querySelector("span:not(.status-dot)")
              ?.textContent || "";
          try {
